...
---
这个功能的使用逻辑是：我点击一个构件：如果这个构件没有HBIM图片信息，插件会向我展示一个按钮“附着图片”。当我点击这个按钮，插件会打开一个文件选择窗口。我选择图片文件之后，插件会处理图片，将这个图片拷贝到相应的文件夹内，并记录这个图片文件的相对路径，作为构件的图片属性，同时把我选择的图片生成缩略图，展示在图片展示区域。如果这个构件已经有了HBIM图像信息，则插件会读出和展示图片的缩略图，并且向我展示一个按钮，当我点击这个按钮，我可以编辑已经录入的图片信息，当我编辑完成后，有一个确定按钮可以让我确定更改的内容。当然，无论哪种情况，都需要一个取消按钮，让插件的状态恢复到这次没有编辑的状态。
