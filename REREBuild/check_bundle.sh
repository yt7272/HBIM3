#!/bin/bash
# HBIM Plugin Bundle 完整性检查
# 用法: ./check_bundle.sh [bundle路径]
# 默认: build/Release/HBIMComponentEntry.bundle

BUNDLE="${1:-build/Release/HBIMComponentEntry.bundle}"
FAIL=0
...
# 必需文件
for f in "Contents/Info.plist" "Contents/PkgInfo" "Contents/MacOS/HBIMComponentEntry"; do
  if [ -f "$BUNDLE/$f" ]; then
    echo "  OK   $f"
  else
    echo "  FAIL $f (缺失)"
...
# Resources
  ...
  # 纯DG面板插件，无需HTML资源，跳过DATA_100.rsrd检查
...
# 可执行文件
if [ -f "$BUNDLE/Contents/MacOS/HBIMComponentEntry" ]; then
  ARCH=$(file "$BUNDLE/Contents/MacOS/HBIMComponentEntry")
  echo "  $ARCH"
...
if [ -f "$BUNDLE/Contents/Info.plist" ]; then
  ...
  if [ "$EXE" != "HBIMComponentEntry" ]; then
    echo "  FAIL CFBundleExecutable 应为 HBIMComponentEntry，当前: $EXE"
    FAIL=1
  else
    echo "  OK   CFBundleExecutable = HBIMComponentEntry"
  fi
  if [ "$LSMIN" != "11.0" ]; then
