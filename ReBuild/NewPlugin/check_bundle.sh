...
# 默认: build/Release/NewPlugin.bundle

BUNDLE="${1:-build/Release/NewPlugin.bundle}"
FAIL=0
...
# 必需文件
for f in "Contents/Info.plist" "Contents/PkgInfo" "Contents/MacOS/NewPlugin"; do
  if [ -f "$BUNDLE/$f" ]; then
    echo "  OK   $f"
  else
    echo "  FAIL $f (缺失)"
...
# 可执行文件
if [ -f "$BUNDLE/Contents/MacOS/NewPlugin" ]; then
  ARCH=$(file "$BUNDLE/Contents/MacOS/NewPlugin")
  echo "  $ARCH"
...
if [ -f "$BUNDLE/Contents/Info.plist" ]; then
  ...
  if [ "$EXE" != "NewPlugin" ]; then
    echo "  FAIL CFBundleExecutable 应为 NewPlugin，当前: $EXE"
    FAIL=1
  else
    echo "  OK   CFBundleExecutable = NewPlugin"
  fi
  if [ "$LSMIN" != "11.0" ]; then
