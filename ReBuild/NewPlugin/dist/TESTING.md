...
4. `ComponentInfo_GetComponentInfo()` - 获取构件信息
5. `ComponentInfo_SaveComponentInfo()` - 保存构件信息
6. `TestBridge()` - 测试桥接
7. `LogDebug()` - 调试日志
8. `ComponentInfo_SelectPhotos()` - 选择照片
9. `ComponentInfo_GetPhotoFullPath()` - 获取照片完整路径
10. `ComponentInfo_ViewPhotos()` - 查看照片
11. `ComponentInfo_GetPhotoBase64()` - 获取Base64编码照片
12. `ComponentInfo_SetEditMode()` - 设置编辑模式
13. `ComponentInfo_SetUnsavedChanges()` - 设置未保存更改状态
...
#### 问题: 插件版本显示不正确 (显示旧版本)
**可能原因**:
1. CEF (Chromium Embedded Framework) 浏览器缓存导致旧版本HTML被缓存
2. 插件已更新但浏览器缓存未清除

**解决方案**:
1. **清除CEF缓存** (在Archicad中):
   - 打开插件面板
   - 按 **F12** 打开开发者工具
   - 切换到 **Application** (应用程序) 标签页
   - 在左侧选择 **Storage** → **Clear storage** (清除存储)
   - 点击 **Clear site data** (清除站点数据)
   - 关闭开发者工具，关闭并重新打开插件面板

2. **强制重新加载** (在Archicad中):
   - 打开插件面板
   - 按 **Ctrl+Shift+R** (Windows) 或 **Cmd+Shift+R** (macOS) 强制重新加载页面
...
**最后更新**: 2026-02-13  
**插件版本**: 0.2.13.71  
**最近修复**: 修复JavaScript语法错误、版本显示问题、构件信息加载逻辑、按钮显示逻辑，添加“添加信息”按钮、添加键盘快捷键、按钮工具提示、修复照片按钮状态管理、修复未定义变量  
**测试状态**: 待执行

## 12. 用户反馈收集

为帮助系统化收集测试反馈，请使用配套的反馈模板：

```bash
# 查看反馈模板
cat USER_TESTING_FEEDBACK.md

# 编辑反馈模板（使用文本编辑器）
open USER_TESTING_FEEDBACK.md
```
...
**提交反馈**: 填写完成后，可将此文件内容复制到反馈系统中，或直接发送给开发团队。