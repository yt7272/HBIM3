...
# 创建输出目录（解决链接器错误）
echo "📁 确保输出目录存在..."
mkdir -p Release/MergedPlugin.bundle/Contents/MacOS
mkdir -p Release/MergedPlugin.bundle/Contents/Resources
echo "  ✅ 输出目录已创建"
...
echo "📝 检查Info.plist..."
BUNDLE_PATH="Release/MergedPlugin.bundle"
...
sed -i '' "s/Merged Plugin version [0-9.]*, 合并插件/Merged Plugin version ${VERSION}, 合并插件/" "${INFO_PLIST_PATH}"
echo "  ✅ Info.plist更新完成"

# 验证构建结果
echo "✅ 验证构建结果..."
if [ -f "Release/MergedPlugin.bundle/Contents/MacOS/MergedPlugin" ]; then
    echo "  成功：插件已构建"
    echo "  位置：build/Release/MergedPlugin.bundle"
    
    # 检查文件大小
    FILESIZE=$(stat -f%z "Release/MergedPlugin.bundle/Contents/MacOS/MergedPlugin" 2>/dev/null || stat -c%s "Release/MergedPlugin.bundle/Contents/MacOS/MergedPlugin" 2>/dev/null || echo "未知")
    echo "  可执行文件大小：${FILESIZE} 字节"
    
    # 检查架构
    echo "  架构信息："
    file "Release/MergedPlugin.bundle/Contents/MacOS/MergedPlugin" || true
    
    # 列出Bundle内容
    echo "  Bundle结构："
    find "Release/MergedPlugin.bundle" -type f | wc -l | xargs echo "    文件数量："
else
    echo "  ❌ 错误：插件构建失败"
    exit 1
fi
...
if [ -f "../check_bundle.sh" ]; then
    ../check_bundle.sh Release/MergedPlugin.bundle || true
fi
...
echo ""
echo "安装说明："
echo "1. 将 build/Release/MergedPlugin.bundle 复制到Archicad插件目录："
...
echo ""
echo "2. 启动Archicad，在插件管理器中启用 'Merged Plugin'"
echo ""
echo "3. 通过菜单 '测试' → 'Merged Plugin' 打开插件面板"
echo ""
echo "如需重新构建（完全清理）："
echo "  ./build.sh clean"
echo "========================================"