...
// GlobalId Warning Dialog class
class GlobalIdWarningDialog : public DG::ModalDialog,
	...
	public DG::CompoundItemObserver
{
	enum {
		OkButtonId       = 3,
		CancelButtonId   = 4,
		TextEditGlobalIdId = 2,
		LeftTextMessageId = 1
	};

	DG::Button    okButton;
	DG::Button    cancelButton;
	DG::TextEdit  textEditGlobalId;
	DG::LeftText  leftTextMessage;

	GS::UniString enteredGlobalId;

public:
	GlobalIdWarningDialog (GSResModule resModule, short dialogId, GSResModule iconModule);
	virtual void ButtonClicked (const DG::ButtonClickEvent& ev) override;

	GS::UniString GetEnteredGlobalId () const { return enteredGlobalId; }
	bool UserAccepted () const { return enteredGlobalId.IsEmpty () ? false : true; }
};
...
// 手动 GlobalId 用项目附加数据（ModulData）存储，不再创建第4个属性或第二个属性组（CreatePropertyGroup/CreatePropertyDefinition 在此环境受限）
static const char* kPhotoStorageIdModulPrefix = "PhotoStorageId_";

static GS::UniString PhotoStorageIdModulName (const API_Guid& elemGuid)
{
	return GS::UniString (kPhotoStorageIdModulPrefix) + APIGuidToString (elemGuid);
}

static GSErrCode GetPhotoStorageIdFromModulData (const API_Guid& elemGuid, GS::UniString& outGlobalId)
{
	outGlobalId.Clear ();
	GS::UniString modulName = PhotoStorageIdModulName (elemGuid);
	API_ModulData info = {};
	GSErrCode err = ACAPI_ModulData_GetInfo (&info, modulName);
	if (err != NoError || info.dataHdl == nullptr) return err;
	GSSize size = BMhGetSize (info.dataHdl);
	if (size < 4) { BMhKill (&info.dataHdl); return NoError; }
	const char* p = (const char*) *info.dataHdl;
	Int32 len = *reinterpret_cast<const Int32*> (p);
	if (len <= 0 || (GSSize) (4 + len) > size) { BMhKill (&info.dataHdl); return NoError; }
	outGlobalId = GS::UniString (p + 4);
	BMhKill (&info.dataHdl);
	return NoError;
}

static GSErrCode SetPhotoStorageIdToModulData (const API_Guid& elemGuid, const GS::UniString& globalId)
{
	GS::UniString modulName = PhotoStorageIdModulName (elemGuid);
	GS::UniString copy = globalId;
	auto cstrObj = copy.ToCStr ();
	const char* cstr = cstrObj.Get ();
	if (cstr == nullptr) cstr = "";
	Int32 len = (Int32) (strlen (cstr) + 1);
	GSHandle h = BMhAll (4 + len);
	if (h == nullptr) return APIERR_MEMFULL;
	char* p = (char*) *h;
	*reinterpret_cast<Int32*> (p) = len;
	memcpy (p + 4, cstr, (size_t) len);
	API_ModulData md = {};
	md.dataVersion = 1;
	md.platformSign = GS::Act_Platform_Sign;
	md.dataHdl = h;
	GSErrCode err = ACAPI_ModulData_Store (&md, modulName);
	BMhKill (&h);
	return err;
}

// Helper function to show GlobalId warning dialog and store manual GlobalId
static bool ShowGlobalIdWarningDialogAndStore (const API_Guid& elementGuid, GS::UniString& outGlobalId)
{
	...
	if (err != NoError) {
		// Try to ensure they exist
		err = ComponentInfo::EnsurePropertyGroupAndDefinitions (guids);
		if (err != NoError) {
			ACAPI_WriteReport ("无法创建属性定义，无法存储手动GlobalId。", true);
			return false;
		}
	}
	
	GSResModule resMod = ACAPI_GetOwnResModule ();
	GlobalIdWarningDialog dlg (resMod, 32540, resMod);
	...
	outGlobalId = dlg.GetEnteredGlobalId ();
	if (outGlobalId.IsEmpty ()) {
		// User clicked OK but left field empty
		ACAPI_WriteReport ("GlobalId不能为空。", true);
		return false;
	}
	
	// Store manual GlobalId property
	err = ComponentInfo::SetGlobalIdManualProperty (elementGuid, guids.globalIdManualGuid, outGlobalId);
	if (err != NoError) {
		ACAPI_WriteReport ("存储手动GlobalId属性失败。", true);
		return false;
	}
	
	ACAPI_WriteReport ("已存储手动GlobalId属性。", false);
	return true;
}
...
static GSErrCode FindOrCreateDefinition (const API_PropertyGroup& group, const GS::UniString& name, API_PropertyDefinition& outDef, API_VariantType valueType, API_PropertyCollectionType collType, const GS::Array<API_Guid>& allClassificationItems)
{
	...
	if (err != NoError) {
		ACAPI_WriteReport ("FindOrCreateDefinition [%s]: CreatePropertyDefinition 失败: %s (err=%d)", true, name.ToCStr ().Get (), ErrID_To_Name (err), (int) err);
	}
	return err;
}
...
GSErrCode ComponentInfo::EnsurePropertyGroupAndDefinitions (PropertyDefinitionGuids& outGuids)
{
	...
	// 手动 GlobalId 用 ModulData 存储，不再创建第4个属性或第二个属性组
	outGuids.globalIdManualGuid = APINULLGuid;
	return NoError;
}
...
GSErrCode ComponentInfo::DeleteAndRecreatePropertyGroup (PropertyDefinitionGuids& outGuids)
{
	...
	for (UInt32 i = 0; i < groups.GetSize (); ++i) {
		if (groups[i].name == kGroupName) {
			ACAPI_WriteReport ("DeleteAndRecreatePropertyGroup: 删除属性组 %s", false, groups[i].name.ToCStr ().Get ());
...
GSErrCode ComponentInfo::FindPropertyDefinitions (PropertyDefinitionGuids& outGuids)
{
	...
	// globalIdManualGuid 固定为 APINULLGuid，手动 GlobalId 用 ModulData 存储
...
static GS::UniString GetGlobalIdPropertyValue (const API_Guid& elementGuid)
{
	GS::UniString globalIdValue;
	
	try {
		// Use PropertyUtils to get all IFC properties
		GS::Array<PropertyUtils::PropertyInfo> properties = PropertyUtils::GetAllIFCPropertiesForElement (elementGuid);
		
		// Search for GlobalId, Tag, ID, Identifier (in order of priority)
		for (const PropertyUtils::PropertyInfo& prop : properties) {
			GS::UniString propName = prop.propertyName;
			GS::UniString propValue = prop.propertyValue;
			
			if (globalIdValue.IsEmpty ()) {
				if (propName.IsEqual ("GlobalId", GS::CaseInsensitive) ||
					propName.IsEqual ("globalId", GS::CaseInsensitive) ||
					propName.IsEqual ("GLOBALID", GS::CaseInsensitive) ||
					propName.IsEqual ("GlobalID", GS::CaseInsensitive)) {
					globalIdValue = propValue;
					break; // Highest priority, stop searching
				}
			}
		}
		
		// If GlobalId not found, try Tag
		if (globalIdValue.IsEmpty ()) {
			for (const PropertyUtils::PropertyInfo& prop : properties) {
				GS::UniString propName = prop.propertyName;
				GS::UniString propValue = prop.propertyValue;
				
				if (propName.IsEqual ("Tag", GS::CaseInsensitive) ||
					propName.IsEqual ("tag", GS::CaseInsensitive) ||
					propName.IsEqual ("TAG", GS::CaseInsensitive) ||
					propName.IsEqual (GS::UniString ("名称"), GS::CaseInsensitive) ||
					propName.IsEqual (GS::UniString ("名稱"), GS::CaseInsensitive) ||
					propName.IsEqual (GS::UniString ("标签"), GS::CaseInsensitive)) {
					globalIdValue = propValue;
					break;
				}
			}
		...
		// If still not found, try ID and Identifier
		if (globalIdValue.IsEmpty ()) {
			for (const PropertyUtils::PropertyInfo& prop : properties) {
				GS::UniString propName = prop.propertyName;
				GS::UniString propValue = prop.propertyValue;
				
				if (propName.IsEqual ("ID", GS::CaseInsensitive) ||
					propName.IsEqual ("Id", GS::CaseInsensitive) ||
					propName.IsEqual ("Identifier", GS::CaseInsensitive)) {
					globalIdValue = propValue;
					break;
				}
			}
...
static GS::UniString SanitizeFolderName (const GS::UniString& input)
{
	GS::UniString result = input;
	
	// Replace characters that are invalid in file system paths
	static const GS::UniString invalidChars = "/\\:*?\"<>|";
	for (UInt32 i = 0; i < invalidChars.GetLength (); ++i) {
		GS::uchar_t ch = invalidChars.GetChar (i);
		result.ReplaceAll (ch, '_');
	}
	...
	const UInt32 maxLength = 100;
	if (result.GetLength () > maxLength) {
		result.Delete (maxLength, result.GetLength () - maxLength);
	}
...
GS::UniString ComponentInfo::GlobalIdToFolderName (const API_Guid& elementGuid)
{
	...
	if (globalId.IsEmpty ()) {
		// Try manual GlobalId（属性或 ModulData）
		PropertyDefinitionGuids guids;
		if (FindPropertyDefinitions (guids) == NoError) {
			GS::UniString manualGlobalId;
			if (GetGlobalIdManualProperty (elementGuid, guids.globalIdManualGuid, manualGlobalId) == NoError && !manualGlobalId.IsEmpty ()) {
				globalId = manualGlobalId;
			}
		}
	}
	
	if (globalId.IsEmpty ()) {
		// Return empty string to indicate missing GlobalId
		return GS::EmptyUniString;
	}
	
	return SanitizeFolderName (globalId);
}
...
GSErrCode ComponentInfo::GetComponentPhotoFolder (const API_Guid& elemGuid, IO::Location& outComponentPhotoFolder, IO::Location& outProjectFolderLoc, GS::UniString& outProjectName)
{
	...
	GS::UniString globalIdFolderName = GlobalIdToFolderName (elemGuid);
	if (globalIdFolderName.IsEmpty ()) {
		// GlobalId missing - show warning dialog to allow manual entry
		GS::UniString manualGlobalId;
		if (!ShowGlobalIdWarningDialogAndStore (elemGuid, manualGlobalId)) {
			// User cancelled or entered empty GlobalId
			ACAPI_WriteReport ("操作取消：构件缺少GlobalId属性，无法创建照片文件夹。", true);
			return APIERR_GENERAL;
		}
		// Manual GlobalId stored, retry getting folder name
		globalIdFolderName = GlobalIdToFolderName (elemGuid);
		if (globalIdFolderName.IsEmpty ()) {
			// Should not happen if manual GlobalId stored successfully
			ACAPI_WriteReport ("存储手动GlobalId后仍无法获取文件夹名称，请检查属性定义。", true);
			return APIERR_GENERAL;
		}
	}
	outComponentPhotoFolder = photoRootLoc;
	err = outComponentPhotoFolder.AppendToLocal (IO::Name (globalIdFolderName));
	if (err != NoError) return err;
...
GSErrCode ComponentInfo::CopyPhotosForComponent (const API_Guid& elemGuid, const GS::Array<IO::Location>& sourceLocs, GS::Array<GS::UniString>& outRelPaths, UInt32 startIndex)
{
	...
	GS::UniString globalIdFolderName = GlobalIdToFolderName (elemGuid);
	if (globalIdFolderName.IsEmpty ()) {
		// This should not happen if GetComponentPhotoFolder succeeded, but check anyway
		ACAPI_WriteReport ("构件缺少GlobalId属性，无法生成照片路径前缀。", true);
		return APIERR_GENERAL;
	}
	GS::UniString prefix = projectName + kPhotoFolderSuffix + "/" + globalIdFolderName + "/";
	outRelPaths.Clear ();
...
GSErrCode ComponentInfo::GetGlobalIdManualProperty (API_Guid elemGuid, API_Guid globalIdManualDefGuid, GS::UniString& outGlobalId)
{
	outGlobalId.Clear ();
	if (globalIdManualDefGuid != APINULLGuid) {
		API_Property p = {};
		GSErrCode err = ACAPI_Element_GetPropertyValue (elemGuid, globalIdManualDefGuid, p);
		if (err == NoError && p.status == API_Property_HasValue && p.value.variantStatus == API_VariantStatusNormal) {
			outGlobalId = p.value.singleVariant.variant.uniStringValue;
			return NoError;
		}
	}
	return GetPhotoStorageIdFromModulData (elemGuid, outGlobalId);
}

GSErrCode ComponentInfo::SetGlobalIdManualProperty (API_Guid elemGuid, API_Guid globalIdManualDefGuid, const GS::UniString& globalId)
{
	if (globalIdManualDefGuid != APINULLGuid) {
		API_PropertyDefinition def = {};
		def.guid = globalIdManualDefGuid;
		...
		if (err == NoError) {
			...
			p.value.variantStatus = API_VariantStatusNormal;
			p.value.singleVariant.variant.type = API_PropertyStringValueType;
			p.value.singleVariant.variant.uniStringValue = globalId;
			err = ACAPI_Element_SetProperty (elemGuid, p);
			if (err == NoError) return NoError;
		}
	}
	return SetPhotoStorageIdToModulData (elemGuid, globalId);
}
...
GS::UniString ComponentInfo::GetAddOnVersionString ()
{
...
int ComponentInfo::GetAddOnVersionMajor ()
{
...
int ComponentInfo::GetAddOnVersionMinor ()
{
...
int ComponentInfo::GetAddOnVersionPatch ()
{
...
int ComponentInfo::GetAddOnVersionBuild ()
{
...
// GlobalIdWarningDialog implementation
GlobalIdWarningDialog::GlobalIdWarningDialog (GSResModule resModule, short dialogId, GSResModule iconModule) :
	DG::ModalDialog (resModule, dialogId, iconModule),
	okButton (GetReference (), OkButtonId),
	cancelButton (GetReference (), CancelButtonId),
	textEditGlobalId (GetReference (), TextEditGlobalIdId),
	leftTextMessage (GetReference (), LeftTextMessageId)
{
	AttachToAllItems (*this);
	enteredGlobalId.Clear ();
}

void GlobalIdWarningDialog::ButtonClicked (const DG::ButtonClickEvent& ev)
{
	if (ev.GetSource () == &okButton) {
		enteredGlobalId = textEditGlobalId.GetText ();
