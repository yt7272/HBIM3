...
{
	const GS::UniString pictNameStr (pictName);
	outs << "\tname = " << pictNameStr.ToCStr ().Get ();
	if (strlen (mime) != 0)
		outs << " (" << mime << ")";
	outs << endl;
					...
					const void* par1, const void* par2, const void* /*par3*/)
{
	...
	switch (primElem->header.typeID) {
		...
		case API_PrimCtrl_FrameBegID:
			vlist << "+ API_PrimCtrl_FrameBegID" << endl;
			break;

		case API_PrimCtrl_FrameEndID:
			vlist << "- API_PrimCtrl_FrameEndID" << endl;
			break;
	}
