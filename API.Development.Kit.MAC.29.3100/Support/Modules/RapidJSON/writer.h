...
public:
    ...
    static const int kDefaultMaxDecimalPlaces = 10;
    ...
    */
	...
	bool WriteStackIsEmpty () const {
		return level_stack_.Empty ();
	};
...
protected:
    ...
    bool WriteDouble(double d) {
        if (internal::Double(d).IsNanOrInf()) {
            if ((writeFlags & kWriteNanAndInfFlag) == 0)
                return false;
		...
		switch (maxDecimalPlaces_) {
			...
			case 9:  p = 1000000000; break;
			case 10: p = 10000000000; break;
			default:
				p = pow (10, maxDecimalPlaces_);
			break;
		}
        d = std::round (d * p) / p;
...
#undef Z16
        ...
        while (ScanWriteUnescapedString(is, length)) {
            ...
            else if (RAPIDJSON_UNLIKELY(!((writeFlags & kWriteValidateEncodingFlag) != 0 ?
...
inline bool Writer<StringBuffer>::WriteDouble(double d) {
    if (internal::Double(d).IsNanOrInf()) {
        ...
        if ((kWriteDefaultFlags & kWriteNanAndInfFlag) == 0)
            return false;
        if (internal::Double(d).IsNan()) {
            PutReserve(*os_, 3);
            PutUnsafe(*os_, 'N'); PutUnsafe(*os_, 'a'); PutUnsafe(*os_, 'N');
