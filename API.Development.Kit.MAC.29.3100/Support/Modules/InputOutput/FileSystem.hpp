...
public:
	...
	public:
		struct MountInfo {
			Location  mountPoint;
			FixedVFS* fixedVFS = nullptr;
		};

		FixedVFSTree ();

		void		Add		 (const Location& mountPoint, FixedVFS* fixedVFS);
		void		Delete	 (const Location& mountPoint);
		bool		Contains (const Location& mountPoint) const;
		bool		Contains (const FixedVFS* fixedVFS) const;

		Location	FindMountPoint		(const FixedVFS* fixedVFS) const;
		MountInfo	FindOwnerMountPoint (const Location& loc) const;

		GS::Array<Location>	GetMountPoints () const;
		GS::Array<Location>	GetMountPoints (const Location& fromRoot) const;
	...
	private:
		struct Name {
			...
			Name (const IO::Name& name, bool isNetworkName);
		...
		struct Node {
			Name					name;
			GS::Array<Owner<Node>>	children;
			Node*					parent = nullptr;
			FixedVFS*				fixedVFS = nullptr;
			UInt32					referenceCount = 0;

			Node (const Name& name, Node* parent);
		};

		Node	root;

		static GS::Array<Name>	ToNameList		   (const Location& loc);
		static const Node*		FindNode		   (const Node* root, const Location& loc);
		static const Node*		FindMountPointNode (const Node* root, const FixedVFS* fixedVFS);
		static void				GetMountPointNodes (const Node* root, GS::Array<const Node*>& mountPointNodes);
		static Location			GetLocation		   (const Node* start, const Node* end);
	};

	FixedVFSTree						fixedVFSTree;						// stores fixed virtual file systems
	GS::HashSet<RollingVFS*>			rollingVFSSet;						// stores rolling virtual file systems
	...
	UInt32								accessDeniedRetryDuration = 1000;	// duration in milliseconds while an operation that has been failed due to access denied is retried
	const UInt32						AccessDeniedWaitTime = 100;			// wait time in milliseconds between two trials of an operation that has been failed due to access denied

	FileSystem () = default;
	FileSystem (const FileSystem& source) = delete;
	FileSystem&	operator= (const FileSystem& source) = delete;

	FixedVFS*			FindFixedVFS   (const Location& loc) const;
	RollingVFS*			FindRollingVFS (const Location& loc) const;
...
public:
	...
	GSErrCode	Mount   (FixedVFS& fixedVFS, const Location& mountPoint);
	GSErrCode	Mount   (RollingVFS& rollingVFS);

	GSErrCode	Unmount (FixedVFS& fixedVFS);
	GSErrCode	Unmount (RollingVFS& rollingVFS);
