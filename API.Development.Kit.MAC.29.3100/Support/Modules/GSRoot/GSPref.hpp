...
GSROOT_DLL_EXPORT
GS::ErrCode			SetLongValue	   (const char*				path, 
										const char*				iValueName, 
										const Int32				iData);

GSROOT_DLL_EXPORT
GS::ErrCode			SetDoubleValue	   (const char*				path, 
										const char*				iValueName, 
										const double			iData);

GSROOT_DLL_EXPORT
GS::ErrCode			SetBoolValue	   (const char*				path, 
										const char*				iValueName, 
										const bool				iData);

GSROOT_DLL_EXPORT
GS::ErrCode			SetStringValue	   (const char*				path, 
										const char*				iValueName, 
										const GS::UniString&	iData);
