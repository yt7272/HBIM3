...
public:
	...
	ConstForwardContainerIterator ();
	ConstForwardContainerIterator (const Cont& targetContainer, const State& initialState);
	ConstForwardContainerIterator (const ConstForwardContainerIterator& source);
	ConstForwardContainerIterator (const ForwardContainerIterator<Cont, State, Type>& source);
	template <class SourceState, class SourceType>
	ConstForwardContainerIterator (const ConstForwardContainerIterator<Cont, SourceState, SourceType>& source);

	ConstForwardContainerIterator&	operator= (const ConstForwardContainerIterator& source);
	ConstForwardContainerIterator&	operator= (const ForwardContainerIterator<Cont, State, Type>& source);
	template <class SourceState, class SourceType>
	ConstForwardContainerIterator&	operator= (const ConstForwardContainerIterator<Cont, SourceState, SourceType>& source);
	...
	ConstForwardContainerIterator&	operator++ ();
	ConstForwardContainerIterator	operator++ (int);

	bool							operator== (const ConstForwardContainerIterator& rightOp) const;
	bool							operator!= (const ConstForwardContainerIterator& rightOp) const;

	template <class SourceState, class SourceType>
	bool							operator== (const ConstForwardContainerIterator<Cont, SourceState, SourceType>& rightOp) const;
	template <class SourceState, class SourceType>
	bool							operator!= (const ConstForwardContainerIterator<Cont, SourceState, SourceType>& rightOp) const;
...
template <class Cont, class State, class Type>
template <class SourceState, class SourceType>
bool	ConstForwardContainerIterator<Cont, State, Type>::operator== (const ConstForwardContainerIterator<Cont, SourceState, SourceType>& rightOp) const
{
	PRECOND_DB (cont != nullptr);
	PRECOND_DB (cont == rightOp.cont);
...
template <class Cont, class State, class Type>
struct std::iterator_traits<GS::ForwardContainerIterator<Cont, State, Type>> {
...
template <class Cont, class State, class Type>
struct std::iterator_traits<GS::ConstForwardContainerIterator<Cont, State, Type>> {
	typedef		   forward_iterator_tag					 iterator_category;
...
template <class Cont, class State, class Type>
struct std::iterator_traits<GS::GeneralForwardContainerIterator<Cont, State, Type>> {
	typedef forward_iterator_tag						 iterator_category;
	typedef Type										 value_type;
	typedef GS::IteratorOffset							 difference_type;
	typedef Type*										 pointer;
...
template <class Cont, class State, class Type>
struct std::iterator_traits<GS::ConstGeneralForwardContainerIterator<Cont, State, Type>> {
	typedef		   forward_iterator_tag					 iterator_category;
	typedef const Type									 value_type;
	typedef		  GS::IteratorOffset					 difference_type;
	typedef const Type*									 pointer;
