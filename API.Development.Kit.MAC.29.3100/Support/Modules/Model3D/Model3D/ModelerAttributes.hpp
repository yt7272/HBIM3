...
{
	PRECOND_T (multithreadingMode == SingleThreaded);
	PRECOND_T (index > parentUFILCount);
...
{
	PRECOND_T (multithreadingMode == SingleThreaded)
	PRECOND_T (index > parentTextureCount);
...
{
	PRECOND_T (multithreadingMode == SingleThreaded);
	PRECOND_T (index > parentUMATCount);
...
{
	PRECOND_T (0 < index && index <= GetFillCount ());
...
{
	PRECOND_T (0 < index && index <= GetTextureCount ());
...
{
	PRECOND_T (0 < index && index <= GetMaterialCount ());
