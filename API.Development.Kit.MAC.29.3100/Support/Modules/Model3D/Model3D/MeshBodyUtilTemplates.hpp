...
//This function is needed beacues the GSModeller status bit is not equivalent to the GDL status bit and this can cause error messages during conversion
inline STATL ConvertgsModelerStatusToGDLStatus (STATL GSMStatus)
{
	return GSMStatus &= GDL_PgonStatus::PgonStatusUsedBits;
}
	...
	GSAttributeIndex&							polyIumat)
{
	...
	for (ULong j = 0; j < body.GetPolygonCount () && !processControl.TestBreak (); ++j) {
		...
		STATL pstatGDL = ConvertgsModelerStatusToGDLStatus (pstat);
		...
		try {
			...
			if (GetTextureInfo != nullptr && irtxt > 0 && GetTextureInfo (userData, j, irtxt, &name, &width, &height, &hasAlpha) == NoError) {
				errCode |= channelWriter.WriteLine (GS::UniString::Printf ("pipg\t\"%s\", %12.*G, %12.*G, %1d,\t\t\t\t\t!\t#%-3u",
					...
					hasAlpha ? 1 : 0, j + 1));
				errCode |= channelWriter.WriteLine (GS::UniString::Printf ("\t\t%4d, %4d, %6u,", lpedg - fpedg + 1, ivect, pstatGDL));
			} else {
				if (tproj == Wrapping::NoWrapping) {
					errCode |= channelWriter.WriteLine (GS::UniString::Printf ("pgon\t%4d, %4d, %6u,\t\t\t\t\t!\t#%-3u PolyId=%d",
						lpedg - fpedg + 1, ivect, pstatGDL, j + 1, polyId));
				} else {
					if (body.GetConstPolygonAttributes (j).HasTextureCoordinateSystem ()) {
						...
						errCode |= channelWriter.WriteLine (GS::UniString::Printf (
							" %12.*G, %12.*G, %12.*G,	!	#%-3u PolyId=%d",
							MESHBODY_WRITE_NUM_PREC, e3.x, MESHBODY_WRITE_NUM_PREC, e3.y, MESHBODY_WRITE_NUM_PREC, e3.z, j + 1, polyId));
						...
						errCode |= channelWriter.WriteLine (GS::UniString::Printf ("pgon{3}    %4d, %4d, %6u, %4d, %4d,				!	#%-3u PolyId=%d",
							lpedg - fpedg + 1, ivect, pstatGDL, (int)tproj.GetMethod (), (int)tproj.GetFlags (), j + 1, polyId));
					}
				}
			}
		}
