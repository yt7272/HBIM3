...
public:
	...
	/**
	 * @brief The key that uniquely identifies a row in the PipeElbowPreferenceTable.
	 * @ingroup MEP
	 * @since Archicad 29
	 */
	struct Key
	{
		/** @brief The referenceId of the row. The table only contains the referenceIds that are defined in the PipeReferenceSet. */
		uint32_t referenceId;

		/** @brief The angle of the row. The table only contains the angles that are defined in the PipeAngleSet. Needs to be between [1, 90] degrees. */
		double angle;
	};
	...
	/**
	 * @brief A row is considered valid if its diameter and radius contains non-zero values, description is optional.
	 * @param key The key that uniquely identifies a row in the PipeElbowPreferenceTable.
	 * @return Returns error if the referenceId and angle is not in the PipeReferenceSet and PipeAngleSet, otherwise returns the validity state of the row.
	 */
...
/**
 * @brief Modifier of the PipeElbowPreferenceTable.
 * @ingroup MEP
 */
...
public:
	 ...
	 * @return Returns error if the referenceId and angle is not in the PipeReferenceSet and PipeAngleSet, otherwise nothing.
	 */
