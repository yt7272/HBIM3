 ...
 * 		Modification: take a look at @ref DuctBranchPreferenceTable "DuctBranchPreferenceTable"
 */
...
public:
	...
	/**
	 * @brief The key that uniquely identifies a row in the DuctBranchPreferenceTable.
	 * @ingroup MEP
	 * @since Archicad 29
	 */
	struct Key
	{
		/** @brief The main axis referenceId of the row. The table only contains the referenceIds that are defined in the DuctReferenceSet. */
		uint32_t mainAxisReferenceId;

		/** @brief The branch axis angle of the row. The table only contains the angles that are defined in the DuctAngleSet. Needs to be between [1, 90] degrees. */
		double branchAxisAngle;

		/** @brief The branch axis referenceId of the row. The table only contains the referenceIds that are defined in the DuctReferenceSet. */
	...
	*/
	...
	/**
	 * @brief A row is considered valid if both diameters, the length, the branchOffset and the connectionLength contains non-zero values, description is optional.
	 * @param key The key that uniquely identifies a row in the DuctBranchPreferenceTable.
...
/**
 * @brief Modifier of the DuctBranchPreferenceTable.
 * @ingroup MEP
 */
...
public:
	...
	*/
	 ...
	 * @param key The key that uniquely identifies a row in the DuctBranchPreferenceTable.
	 * @return Returns error if a row with he same referenceIds and branch axis angle is already present in the table or if the branch axis referenceId is larger than the main axis referenceId, otherwise the index of the new row.
	 */
	 ...
	 * @return Returns error if at least one referenceId from the key is not in the DuctReferenceSet, or if the branchAxisAngle is not in the DuctAngleSet, or if the value is smaller than the branch axis diameter, otherwise nothing.
	 */
	 ...
	 * @details If no row exists for the given key, but the key contains valid reference Ids and branch angle, it inserts the row.
	 * @return Returns nothing if the operation was successful, or returns error:
	 *			- if at least one referenceId from the key is not in the DuctReferenceSet;
	 *			- if branchAxisAngle from the key is not in the DuctAngleSet;
	 *			- if the value is larger than the already defined main axis diameter;
	 *			- if the value is larger or equal than the already defined length;
	 *			- if branch offset is already defined and the value is larger than 2 * branchOffset;
	 *			- if branch offset and length are already defined and the value is larger than 2 * (length - branchOffset).
	 ...
	 * @param connectionLength The value to be set. Must be a non-zero positive number.
	 * @details If no row exists for the given key, but the key contains valid reference Ids and branch angle, it inserts the row.
	 * @return Returns error if at least one referenceId from the key is not in the DuctReferenceSet, or if the branchAxisAngle is not in the DuctAngleSet, otherwise nothing.
	 */
