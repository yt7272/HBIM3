...
public:
	...
	constexpr Vector3 (const Vector2<Coordinate>, const Coordinate& z);
	...
	constexpr Vector3 (const Vector3dData& src);
	constexpr operator Vector3dData () const;

	template<typename Number2> Vector3& operator= (const Vector3<Number2>& src) { x = src.x; y = src.y; z = src.z; return *this; }

	constexpr void GetCoords (Coordinate* x, Coordinate* y, Coordinate* z) const;
	constexpr void Set (Coordinate x, Coordinate y, Coordinate z);
	...
	constexpr Coordinate	operator* (const Vector3&) const;			// dot product (skalar szorzas)
	constexpr Vector3		operator^ (const Vector3&) const;			// cross product (vektorialis szorzas), !! has low precedence !!

	constexpr Vector3		operator* (const Coordinate&) const;
	constexpr Vector3		operator/ (const Coordinate&) const;

	constexpr const Vector3& operator+= (const Vector3&);
	constexpr const Vector3& operator-= (const Vector3&);
	constexpr const Vector3& operator*= (const Coordinate&);
	constexpr const Vector3&	operator/= (const Coordinate&);
	
	constexpr bool	IsNear (const Vector3&, double = SmallEps) const;
...
template<typename Coordinate>
constexpr Vector3<Coordinate> operator* (const Coordinate&, const Vector3<Coordinate>&);
...
template<typename Coordinate>
constexpr Vector3<Coordinate>::Vector3 (const Vector2<Coordinate> v, const Coordinate& z)
	: x { v.x }, y { v.y }, z { z }
{}

template<typename Coordinate>
constexpr Vector3<Coordinate>::Vector3 (const Vector3dData& src)
	: x { src.x }, y { src.y }, z { src.z }
	{}


template<typename Coordinate>
constexpr Vector3<Coordinate>::operator Vector3dData () const
{
	return Vector3dData { x, y, z };
}


template<typename Coordinate>
constexpr void Vector3<Coordinate>::GetCoords (Coordinate* x, Coordinate* y, Coordinate* z) const
{
...
template<typename Coordinate>
constexpr void Vector3<Coordinate>::Set (Coordinate x, Coordinate y, Coordinate z)
{
	this->x = x;
	this->y = y;
	this->z = z;
...
template<typename Coordinate>
constexpr Coordinate Vector3<Coordinate>::GetLengthSqr (void) const
{
	return x * x + y * y + z * z;
}
...
Vector3<Coordinate> Vector3<Coordinate>::GetUnitVector (void) const
{
	...
	if (l.IsPositive (SmallEps)) {
		return Vector3 { static_cast<Coordinate>(x / l), static_cast<Coordinate>(y / l), static_cast<Coordinate>(z / l) };
	} else {
...
template<typename Coordinate>
constexpr Vector3<Coordinate> Vector3<Coordinate>::operator- () const
{
	return Vector3 { -x, -y, -z };
}

template<typename Coordinate>
constexpr Vector3<Coordinate> Vector3<Coordinate>::operator+ (const Vector3& c) const
{
...
template<typename Coordinate>
constexpr Coordinate Vector3<Coordinate>::operator* (const Vector3& c) const		// dot product (skalar szorzas)
{
	return x * c.x + y * c.y + z * c.z;
}

template<typename Coordinate>
constexpr Vector3<Coordinate> Vector3<Coordinate>::operator^ (const Vector3& c) const	// cross product (vektorialis szorzas), !! has low precedence !!
{
	return Vector3 { y * c.z - z * c.y, z * c.x - x * c.z, x * c.y - y * c.x };
}

template<typename Coordinate>
constexpr Vector3<Coordinate> Vector3<Coordinate>::operator* (const Coordinate& k) const
{
...
template<typename Coordinate>
constexpr const Vector3<Coordinate>& Vector3<Coordinate>::operator+= (const Vector3& c)
{
	x += c.x;
	y += c.y;
	z += c.z;
...
template<typename Coordinate>
constexpr const Vector3<Coordinate>& Vector3<Coordinate>::operator*= (const Coordinate& k)
{
	x *= k;
	y *= k;
	z *= k;
...
template<typename Coordinate>
constexpr bool Geometry::Vector3<Coordinate>::IsNear (const Vector3& c, double eps /*= SmallEps*/) const
{
	return (*this - c).GetLengthSqr () < eps * eps;
}

template<typename Coordinate>
constexpr bool Vector3<Coordinate>::operator== (const Vector3& c) const
{
	return x == c.x && y == c.y && z == c.z;
}
...
template<typename Coordinate>
constexpr inline Vector3<Coordinate> operator* (const Coordinate& k, const Vector3<Coordinate>& c)
{
	return Vector3 { k * c.x, k * c.y, k * c.z };
}
