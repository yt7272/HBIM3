...
// --- NativeImageIcon class -------------------------------------------------------------------------------------------
// NativeImageIcon creates a native HICON/NSImage from a NativeImage, or takes the ownership of an existing
// HICON or NSImage. The native HICON/NSImage is destroyed by the destructor.

class DG_DLL_EXPORT NativeImageIcon: public DG::Icon
{
public:
	NativeImageIcon (const NewDisplay::NativeImage& nativeImage);
	NativeImageIcon (const void* nativeIcon);
	NativeImageIcon (NativeImageIcon&& source);
	NativeImageIcon& operator= (NativeImageIcon&& other);
	virtual ~NativeImageIcon ();

	NativeImageIcon (const NativeImageIcon& other) = delete;
	NativeImageIcon& operator= (const NativeImageIcon& other) = delete;
};
...
public:
	...
	static void			DrawGrayText (NewDisplay::NativeContext& context, const DG::Rect& rect, const GS::UniString& text, short inFontType, short alignment, bool endEllipsis = false);
	...
	static GS::UniString	GetShortcutText		(const DG::Shortcut& shortcut, bool inBracket);
...
namespace VisitorUtility {

enum ApplyContinuation { Interrupt, Continue };

template <class Visitor, class Element>
ApplyContinuation	ApplyOnElement (const Visitor& visitor, Element& element)
{
	using ResultType = std::invoke_result_t<Visitor, Element&>;
	if constexpr (std::is_same_v<ApplyContinuation, ResultType>) {
		return std::invoke (visitor, element);
	} else if constexpr (std::is_same_v<void, ResultType>) {
		std::invoke (visitor, element);
		return ApplyContinuation::Continue;
	} else {
		// Using "AlwaysFalse<>" makes the condition dependent. Using "false" would make the program ill-formed.
		static_assert (GS::AlwaysFalse<Element>, "Unexpected instantiation of ApplyOnElement.");
	}
}
