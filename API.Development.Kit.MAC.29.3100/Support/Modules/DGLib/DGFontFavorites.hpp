...
public:
	...
	void WriteToPrefs (void)
	{
		...
		for (UIndex i = 0; i < fontFamilies.GetSize (); ++i) {
			std::snprintf (favoriteNameId, sizeof (favoriteNameId), "FavoriteName_%ld", (GS::LongForStdio) i);
			session.SetStringValue (favoriteNameId, fontFamilies [i].GetLocalName ());
			std::snprintf (favoriteEncodingId, sizeof (favoriteEncodingId), "FavoriteEncoding_%ld", (GS::LongForStdio) i);
			session.SetLongValue (favoriteEncodingId, fontFamilies [i].GetEncoding (0));
			std::snprintf (favoriteTimeId, sizeof (favoriteTimeId), "FavoriteTime_%ld", (GS::LongForStdio) i);
