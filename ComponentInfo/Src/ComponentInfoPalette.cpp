...
ComponentInfoPalette::ComponentInfoPalette ()
	...
	, btnViewIFCProperties (GetReference (), BtnViewIFCPropertiesId)
	, currentElemGuid (APINULLGuid)
	, currentPhotoIndex (0)
	, isEditMode (false)
{
	ACAPI_WriteReport ("ComponentInfoPalette构造函数: 开始初始化，PaletteResId=%d", false, PaletteResId);
	...
	btnViewIFCProperties.Attach (*this);
	editCompId.Disable ();
	...
	ACAPI_WriteReport ("ComponentInfoPalette构造函数: 初始化完成，控件ID验证: TextComponentIdentifierId=%d, EditCompIdId=%d", false, TextComponentIdentifierId, EditCompIdId);
	UpdateFromSelection ();
}
...
void ComponentInfoPalette::DoViewIFCProperties ()
{
	if (currentElemGuid == APINULLGuid) {
		ACAPI_WriteReport ("请先选择一个构件。", true);
		return;
	...
	IFCPropertiesDialog dlg (resMod, 32530, resMod);
