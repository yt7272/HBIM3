...
echo "📝 检查Info.plist..."
BUNDLE_PATH="Release/HBIMComponentInfoEntry.bundle"
...
# 验证构建结果
echo "✅ 验证构建结果..."
if [ -f "Release/HBIMComponentInfoEntry.bundle/Contents/MacOS/HBIMComponentInfoEntry" ]; then
    echo "  成功：插件已构建"
    echo "  位置：build/Release/HBIMComponentInfoEntry.bundle"
    
    # 检查文件大小
    FILESIZE=$(stat -f%z "Release/HBIMComponentInfoEntry.bundle/Contents/MacOS/HBIMComponentInfoEntry" 2>/dev/null || stat -c%s "Release/HBIMComponentInfoEntry.bundle/Contents/MacOS/HBIMComponentInfoEntry" 2>/dev/null || echo "未知")
    echo "  可执行文件大小：${FILESIZE} 字节"
    
    # 检查架构
    echo "  架构信息："
    file "Release/HBIMComponentInfoEntry.bundle/Contents/MacOS/HBIMComponentInfoEntry" || true
    
    # 列出Bundle内容
    echo "  Bundle结构："
    find "Release/HBIMComponentInfoEntry.bundle" -type f | wc -l | xargs echo "    文件数量："
else
    echo "  ❌ 错误：插件构建失败"
    exit 1
fi
